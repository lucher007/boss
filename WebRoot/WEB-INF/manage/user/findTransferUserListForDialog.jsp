...
var flag = true;
  ...
  //EASYUI FORMATTER
	function formatUserOperation(value,row,index){
		var formatter = '<a class="btn-look" href="javascript:transferUserInfo('+ value +')"><spring:message code="page.look"/></a>';
		return formatter;
	}
  
  function transferUserInfo(id){
		var data = "id="+ id;
		var url="user/getTransferUser?rid="+Math.random();
		$.getJSON(url,data,function(jsonMsg){
			if(jsonMsg.flag=="0"){
			    $.dialog.tips(jsonMsg.error, 1, 'alert.gif');
			}else if(jsonMsg.flag=="1"){
				parent.closeTransferUserDialog();
			}
		});
  }
