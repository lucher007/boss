...
<body>
	<div id="body">
<div class="cur-pos"><spring:message code="page.currentlocation"/>：<spring:message code="menu.product.service"/> &gt; <spring:message code="menu.service.fileimport"/></div>		
		...
		<form method="post" id="searchform" name="searchform" enctype="multipart/form-data">
			<div class="form-box">
				...
				<div class="fb-con">
					<table width="100%" border="0" cellpadding="0" cellspacing="0">
						<tr>
							...
							<td width="25%"><select id="netid" name="netid" class="select" onchange="initServer();">
									<c:forEach items="${service.networkmap}" var="networkMap" varStatus="s">
										<option value="${networkMap.key}" <c:if test="${service.netid == networkMap.key }">selected</c:if>>${networkMap.value}</option>
									</c:forEach>
							</select>
							</td>
						</tr>
				...
				<div class="fb-bom">
					<cite> 
					<span>
						<a href="service/downloadTemplate" class="btn-print"><spring:message code="uploadify.download.template"/></a>
					</span> 
					
					<span>
						<a href="javascript:goBack();" iconCls="icon-back" class="easyui-linkbutton" style="width:auto;"><spring:message code="page.goback"/></a>
<!-- 						<input type="button" class="btn-back" value="<spring:message code="page.goback"/>" onclick="goBack()" /> -->
					</span> 
					<span>
						<a href="javascript:saveService();" iconCls="icon-save" class="easyui-linkbutton" style="width:auto;"><spring:message code="page.save"/></a>
<!-- 						<input type="button" class="btn-save" value="<spring:message code="page.save"/>" onclick="saveService();" /> -->
					</span> 
					</cite> <span class="red">${service.returninfo}</span>
				</div>
			</div>
		</form>
	</div>
	...
	function saveService(){
		$.dialog.confirmMultiLanguage('<spring:message code="fileimport.confirm.emptytable"/>',
			 ...
			 function(){ 
						$("#searchform").attr("action", "service/saveByImportFile").submit();
			}, function(){
						});
	}
	
	function goBack() {
		$("#searchform").attr("action", "service/findByList").submit();
	}
