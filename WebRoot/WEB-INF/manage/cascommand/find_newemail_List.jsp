...
<body>
	<div id="body">
		<div class="cur-pos"><spring:message code="page.currentlocation" />：<spring:message code="menu.authorize.manage" />&gt;<spring:message code="menu.authorize.newemail"/></div>
		<div class="seh-box">
			<form action="" method="post" id="searchForm" name="searchForm">
				<table width="100%">
					<tr>
						...
						<td width="130px">
							<select name="queryversiontype" id="queryversiontype" class="select">
				                ...
				                <option value="gos_pn" <c:if test="${caspnnewemail.queryversiontype == 'gos_pn' }">selected</c:if>><spring:message code="server.versiontype.gos_pn"/></option>
				                <option value="gos_gn" <c:if test="${caspnnewemail.queryversiontype == 'gos_gn' }">selected</c:if>><spring:message code="server.versiontype.gos_gn"/></option>
				            </select>
						</td>
					...
					<tr>
						  ...
						  <td>
						  	<input type="text" id="querydate" name="querydate" value="${caspnnewemail.querydate}" onfocus="WdatePicker({lang:'${locale}',skin:'blueFresh',isShowWeek:true,isShowClear:true,dateFmt:'yyyy-MM-dd'});" class="Wdate inp w150" />
						  </td>
						  <td align="right">
						  	<%-- 
		...
		<div class="lst-box">
			<table id="treetable" class="treetable" width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr class="lb-tit">
					<td><spring:message code="authorize.mailid"/></td>
					<td><spring:message code="server.versiontype"/></td>
					<td><spring:message code="authorize.title"/></td>
					<td><spring:message code="authorize.osd.content"/></td>
					<td><spring:message code="problemcomplaint.addtime"/></td>
					<td><spring:message code="page.operate"/></td>
				</tr>
				
				<c:forEach items="${caspnnewemail.casemaillist }" var="dataList">
					<tr height="30" class="lb-list">
					<c:if test="${dataList.versiontype eq 'gos_pn'}">
						<td>${dataList.id}</td>
					</c:if>
					<c:if test="${dataList.versiontype eq 'gos_gn'}">
						<td>${dataList.emailid}</td>
					</c:if>
						<td><spring:message code="server.versiontype.${dataList.versiontype}"/></td>
						<td>${dataList.emailtitle}</td>
						<td class="remarkClass" title="${dataList.emailcontent}">${fn:substring(dataList.emailcontent, 0, 20)}</td>
		...
		<div class="page">
			<cite><pg:pager url="cascommand/find_newemail_List" items="${caspnnewemail.pager_count}" index="center" maxPageItems="10"
					maxIndexPages="5" isOffset="<%= true %>" export="offset,currentPageNumber=pageNumber" scope="request">
					<pg:param name="index" />
					<pg:param name="maxPageItems"/>
					<pg:param name="maxIndexPages"/>
				<%-- 					
				<pg:param name="querynetid" value="${caspnnewemail.querynetid}"/>
				 --%>					
 					<pg:index>
					<spring:message code="page.total"/>:${caspnnewemail.pager_count}
		...
		function deleteById(id){
			$.dialog.confirmMultiLanguage(
					...
					'<spring:message code="page.cancel"/>', function() {
						$("#searchForm").attr(
								"action",
								"cascommand/delete_newemail?id=" + id
										+ "&pager_offset="
										+ '${caspnnewemail.pager_offset}'
										+ "&rid=" + Math.random());
						$("#searchForm").submit();
					}, function() {
					});
	   ...
	   function cancelById(id){
			$.dialog.confirmMultiLanguage(
					...
					'<spring:message code="page.cancel"/>', function() {
						$("#searchForm").attr(
								"action",
								"cascommand/send_cancel_newemail?id=" + id 
										+ "&pager_offset="
										+ '${caspnnewemail.pager_offset}'
										+ "&rid=" + Math.random());
						$("#searchForm").submit();
		...
		function query() {
			$("#searchForm").attr("action", "cascommand/find_newemail_List");
			$("#searchForm").submit();
		}

		function add() {
			$("#searchForm").attr("action", "cascommand/add_newemail_Init");
			$("#searchForm").submit();
		}
