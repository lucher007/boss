...
<body style="padding:0px; width:99%; ">
			...
			<div class="easyui-panel" title="<spring:message code="authorize.parameter.config"/>" style="width:100%">
				<table width="100%">
					<tr height="25px">
						...
						<td align="right"><spring:message code="newfinger.diaplay.ID"/>：</td>
						<td>
							<input type="text" name="fingerid" id="fingerid" class="inp w200" value="${caspnnewfinger.fingerid}" maxlength="9" onkeyup="value=value.replace(/[^0-9]/g,'')"></input>
						</td>
					</tr>
					<tr height="25px">
						...
						<td><input type="text" id="expiredtime" name="expiredtime"
							...
							class="Wdate inp w150" readonly="readonly" value="${caspnnewfinger.expiredtime}"/>
						</td>
						<td align="right"><spring:message code="authorize.osd.starttime"/>：</td>
						<td><input type="text" id="starttime" name="starttime"
							onfocus="WdatePicker({lang:'${locale}',skin:'blueFresh',isShowWeek:true,isShowClear:true,dateFmt:'yyyy-MM-dd  HH:mm:ss'});"
							class="Wdate inp w150" readonly="readonly" value="${caspnnewfinger.starttime}" />
						</td>
					</tr>
					<tr height="25px">
					...
					<tr height="25px">
						...
						<td >
							<select id="areaiddisplayflag" name="areaiddisplayflag" class="select" style="width:157px;">
									<option value="0" <c:if test="${caspnnewfinger.areaiddisplayflag== '0'}">selected</c:if>><spring:message code="authorize.osd.terminaliddisplayflag.0"/></option>
									<option value="1" <c:if test="${caspnnewfinger.areaiddisplayflag== '1'}">selected</c:if>><spring:message code="authorize.osd.terminaliddisplayflag.1"/></option>
 							</select>
						</td>
					...
					<tr height="25px">
						...
						<td><input name="fonttype" id="fonttype" class="easyui-numberspinner" value="0" data-options="increment:1" style="width:157px;"></input>
						</td>
					</tr>
					<tr>
						...
						<td align="right"><spring:message code="osd.display.frequency"/>：</td>
						<td><input type="text" name="displayfrequency" id="displayfrequency" class="inp w200" value="${caspnnewfinger.displayfrequency}" onkeyup="value=value.replace(/[^0-9]/g,'')"></input>
						</td>
					</tr>
					<tr>
						<td align="right"><spring:message code="osd.display.duration"/>：</td>
						<td><input type="text" name="displaydurationtime" id="displaydurationtime" class="inp w200" value="${caspnnewfinger.displaydurationtime}" onkeyup="value=value.replace(/[^0-9]/g,'')"></input>
						</td>
						<td align="right" width="120px"><spring:message code="newfinger.display.layout"/>：</td>
						<td>
							<select id="layout" name="layout" class="select" >
								<option value="0" <c:if test="${caspnnewfinger.layout== '0'}">selected</c:if>> <spring:message code="newfinger.display.layout0"/></option>
								<option value="1" <c:if test="${caspnnewfinger.layout== '1'}">selected</c:if>><spring:message code="newfinger.display.layout1"/></option>
	 						</select>
						</td>
					</tr>
	...
	<script type="text/javascript">
		function goSend() {
			...
			if ($('#fingerid').val() == '') {
				$.dialog.tips('<spring:message code="newfinger.diaplay.ID.empty"/>',1, 'alert.gif');
				$('#fingerid').focus();
				return;
			}
			
			if ($('#fonttype').val() < 1 || $('#fonttype').val() > 255) {
			...
			if ($('#displayfrequency').val() < 0 || $('#displayfrequency').val() > 255) {
				$.dialog.tips('<spring:message code="newfinger.diaplay.displayfrequency.range"/>',1, 'alert.gif');
				$('#displayfrequency').focus();
				return;
			}
			
			if ($('#displaydurationtime').val() < 10 || $('#displaydurationtime').val() > 65535) {
				$.dialog.tips('<spring:message code="newfinger.diaplay.displaydurationtime.range"/>',1, 'alert.gif');
