...
<body>
	<div id="body">
		<div class="cur-pos"><spring:message code="page.currentlocation" />：<spring:message code="menu.authorize.manage" />&gt;<spring:message code="menu.authorize.forcechangechannel"/></div>
		<div class="seh-box">
					...
					<tr>
						...
						<td width="130px">
							<select name="queryversiontype" id="queryversiontype" class="select">
				                ...
				                <option value="gos_pn" <c:if test="${caspnforcedcc.queryversiontype == 'gos_pn' }">selected</c:if>><spring:message code="server.versiontype.gos_pn"/></option>
				                <option value="gos_gn" <c:if test="${caspnforcedcc.queryversiontype == 'gos_gn' }">selected</c:if>><spring:message code="server.versiontype.gos_gn"/></option>
				            </select>
						</td>
...
<%-- 							<a class="btn-del" href="javascript:deleteById(${dataList.id});"><spring:message code="page.delete"/></a>
 --%>						</td>
    	...
    	<cite>
        	<pg:pager
			    url="cascommand/find_forcedcc_List"
			    items="${caspnforcedcc.pager_count}"
			    index="center"
			    maxPageItems="10"
			    maxIndexPages="5"
			    isOffset="<%= true %>"
				...
				<pg:param name="queryversiontype" value="${caspnforcedcc.queryversiontype}"/>
				<pg:param name="querydate" value="${caspnforcedcc.querydate}"/>
				<pg:index>
		...
		function deleteById(id){
			$.dialog.confirmMultiLanguage(
					...
					'<spring:message code="page.cancel"/>', function() {
						$("#searchForm").attr(
								"action",
								"cascommand/delete_forcedcc?id=" + id
										+ "&pager_offset="
	   ...
	   function cancelById(id){
			$.dialog.confirmMultiLanguage(
					...
					'<spring:message code="page.cancel"/>', function() {
						$("#searchForm").attr(
								"action",
								"cascommand/send_cancel_forcedcc?id=" + id 
										+ "&pager_offset="
										+ '${caspnforcedcc.pager_offset}'
										+ "&rid=" + Math.random());
						$("#searchForm").submit();
		...
		function query() {
			$("#searchForm").attr("action", "cascommand/find_forcedcc_List");
			$("#searchForm").submit();
		}

		function add() {
			$("#searchForm").attr("action", "cascommand/add_forcedcc_Init");
			$("#searchForm").submit();
		}
