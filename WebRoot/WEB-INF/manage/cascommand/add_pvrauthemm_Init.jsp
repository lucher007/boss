...
<body>
	<div class="form-box">
	  <form method="post" id="paramForm" name="paramForm">
	    <div class="cur-pos"><spring:message code="page.currentlocation" />：<spring:message code="menu.authorize.manage" />&gt;<spring:message code="menu.authorize.pvrentitle"/></div>
	    <div class="fb-con">
	      <div class="cur-pos">
	            ...
	            <td align="right"><spring:message code="server.versiontype"/>：</td>
	             <td>
	             	<select name="versiontype" id="versiontype" class="select" onchange="changeVersiontype()">
		                <option value="gos_pn" <c:if test="${pvrAuthEmm.versiontype == 'gos_pn' }">selected</c:if>><spring:message code="server.versiontype.gos_pn"/></option>
		                <option value="gos_gn" <c:if test="${pvrAuthEmm.versiontype == 'gos_gn' }">selected</c:if>><spring:message code="server.versiontype.gos_gn"/></option>
		            </select>
	             </td>
	          </tr>
	        </table>
	      ...
	      <div id="info">
	      	<iframe id="pvrAuthEmm" scrolling="auto" frameborder="0" style="width:100%;height:580px;"></iframe>
	      </div>
	       </form>
	    </div>
	    <!--
	...
	<script type="text/javascript">
		...
		function goBack(){
			$("#paramForm").attr("action", "cascommand/find_pvrAuthEmm_List");
			$("#paramForm").submit();
		}
		...
		$(function() {
			cardid_optionInit();
			changeVersiontype();
			var returninfo = '${caspnpvrAuthEmm.returninfo}';
			if (returninfo != '') {
		...
		function changeVersiontype() {
		    $("#pvrAuthEmm").attr("src","<%=request.getContextPath()%>/cascommand/add_pvrauthemm_Info?versiontype=" + $('#versiontype').val());
		}
