	...
	<div id="body">
			...
			<form action="" method="post" id="searchForm" name="searchForm">
				<table width="100%">
					<tr>
						...
						<td width="130px">
							<select name="queryversiontype" id="queryversiontype" class="select">
				                ...
				                <option value="gos_pn" <c:if test="${caspnproductplay.queryversiontype == 'gos_pn' }">selected</c:if>><spring:message code="server.versiontype.gos_pn"/></option>
				                <option value="gos_gn" <c:if test="${caspnproductplay.queryversiontype == 'gos_gn' }">selected</c:if>><spring:message code="server.versiontype.gos_gn"/></option>
				            </select>
						</td>
					...
					<tr>
						  ...
						  <td>
						  	<input type="text" id="querydate" name="querydate" value="${caspnproductplay.querydate}" onfocus="WdatePicker({lang:'${locale}',skin:'blueFresh',isShowWeek:true,isShowClear:true,dateFmt:'yyyy-MM-dd'});" class="Wdate inp w150" />
						  </td>
						  <td align="right">
						  	<%-- 
		...
		<div class="lst-box">
			<table id="treetable" class="treetable" width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr class="lb-tit">
					<td><spring:message code="productplay.playid" /></td>
				...
				<c:forEach items="${caspnproductplay.caspnproductplaylist }" var="dataList">
					<tr height="30" class="lb-list">
<!-- 					<c:if test="${dataList.versiontype eq 'gos_pn'}"> -->
<!-- 						<td>${dataList.id}</td> -->
<!-- 					</c:if> -->
						<c:if test="${dataList.versiontype eq 'gos_pn'}">
							<td>${dataList.playid}</td>
						</c:if>
...
<!--  								<a class="btn-scal" href="javascript:cancelById(${dataList.playid});"><spring:message code="authorize.osd.cancel"/></a> -->
<!-- 						    </c:if> -->
 						</td>
						<%-- 						
		...
		<div class="page">
			<cite><pg:pager url="cascommand/find_newemail_List" items="${caspnproductplay.pager_count}" index="center" maxPageItems="10"
				...
				<pg:param name="querynetid" value="${caspnnewemail.querynetid}"/>
				 --%>					
 					<pg:index>
					<spring:message code="page.total"/>:${caspnproductplay.pager_count}
	   ...
	   function cancelById(id){
			$.dialog.confirmMultiLanguage(
					...
					'<spring:message code="page.cancel"/>', function() {
						$("#searchForm").attr(
								"action",
								"cascommand/send_cancel_productplayallow?id=" + id 
										+ "&pager_offset="
										+ '${caspnproductplay.pager_offset}'
										+ "&rid=" + Math.random());
						$("#searchForm").submit();
					}, function() {
					});
		...
		function query() {
			$("#searchForm").attr("action", "cascommand/find_productplayallow_List");
			$("#searchForm").submit();
		}

		function add() {
			$("#searchForm").attr("action", "cascommand/add_productplayallow_Init");
			$("#searchForm").submit();
		}
