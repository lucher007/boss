...
<div id="body">
	...
	%>
	<div class="cur-pos"><spring:message code="page.currentlocation" />：<spring:message code="menu.stat.manage" /> &gt; <spring:message code="menu.stat.userOfflineStat" /></div>
   ...
   <div class="lst-box">
	 <div id="userOfflineStat" style="width:auto"></div>
   </div>
</div>
...
<script type="text/javascript">
	...
	function initUserOfflineStat(){
	    $('#userOfflineStat').datagrid({
	         title: '<spring:message code="menu.stat.userOfflineStat" />',
	         url:'statreport/userOfflineStatJson',
	     ...
	     var p = $('#userOfflineStat').datagrid('getPager');
	     $(p).pagination({
	...
	function getSelected(){
		var row = $("#userOfflineStat").datagrid('getSelected');
		...
		if (row){
	    	...
	    	operatorReportForDialog = $.dialog({
		    	title: '<spring:message code="statreport.userOfflineStatdetail" />('+row.netname+'_'+row.productname+')',
		    	lock: true,
		    	width: 900,
		    	height: 530,
		    	top: 0,
		    	...
		    	max: false,
		    	min: false,
		    	content: 'url:statreport/userOfflineStatdetail?querynetid='+querynetid+'&queryareacode='+queryareacode+'&querystarttime='+querystarttime+'&queryendtime='+queryendtime+'&queryproductid='+row.productid
				});
		}
	}
    ...
    function saveDownloadEventFile(){
		$.dialog.confirmMultiLanguage('<spring:message code="page.confirm.execution"/>?', 
 		        ...
 		        function(){ 
 					$("#searchForm").attr("action", "statreport/exportUserOfflineStatExcel");
					$("#searchForm").submit();
 				}, function(){
 							});
	}
	...
	function queryUserproductAudienceStat(){
		$('#userOfflineStat').datagrid('reload',paramsJson());
	}
