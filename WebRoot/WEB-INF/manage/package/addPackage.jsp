...
<body>
	<div id="body">
		<div class="cur-pos"><spring:message code="page.currentlocation" />：<spring:message code="menu.product.package" />&gt;<spring:message code="package.manage.add"/></div>
		...
		<form action="" method="post" id="packageForm" name="packageForm">
			...
			<div class="easyui-panel" title="<spring:message code="authorize.parameter.config"/>" style="width:950px">
				...
				<tr height="30px">
						...
						<td width="30%">
							 <select id="netid" name="netid" class="select" style="width: 200px;" onchange="cleanPackageJson();">
							 	 <option value=""><spring:message code="page.select"/></option>
								 <c:forEach items="${package.networklist}" var="networklist" varStatus="s">
				                   <option value="${networklist.id}" <c:if test="${package.netid == networklist.id }">selected</c:if>>${networklist.netname}</option>
				                </c:forEach>
				             </select> <span class="red">*</span>
						</td>
						<td align="right"><spring:message code="package.name"/>：</td>
						<td>
					          <input type="text" class="inp" name="name" id="name" style="width: 300px;" value="${package.name}" /> <span class="red">*</span>
						</td>
				...
				<tr height="30px">
						...
						<td>
							<input type="text" id="starttime" name="starttime" onclick="WdatePicker({lang:'${locale}',skin:'blueFresh',isShowWeek:true,isShowClear:true,dateFmt:'yyyy-MM-dd'});" class="Wdate inp w200" style="width: 180px;" value="${package.starttime}"/>
							<span class="red">*</span>
						</td>
					
						<td align="right"><spring:message code="package.endtime"/>：</td>
						<td>
							<input type="text" id="endtime" name="endtime" onclick="WdatePicker({lang:'${locale}',skin:'blueFresh',isShowWeek:true,isShowClear:true,dateFmt:'yyyy-MM-dd'});" class="Wdate inp w200" style="width: 180px;" value="${package.endtime}"/>
							<span class="red">*</span>
						</td>
				</tr>
	...
	var areaDialog;
	var packageJson ={"rows":[], "footer":[{"name":'<spring:message code="package.treegrid.totalprice"/>',"totalprice":0.00}]};
    ...
    //保存套餐
 	function savePackage() {
		...
		if ($('#starttime').val() == '' || $('#endtime').val() == '') {
	 	...
	 	$("#packageForm").attr("action", "package/save").submit();
	};
	...
	function saveStb(obj){   //2222
		...
		function saveProductPackage(obj){   //4444
		       ...
		       if(rows == null || rows == '' || rows.length == 0){
		    	   //$.dialog.tips('<spring:message code="package.product.empty"/>', 2, 'alert.gif');
		    	   return;
		       }
			   ...
			   for(var i=0; i<rows.length; i++){
					...
					product['detail'] = '<spring:message code="package.productpackage.price" />:' + product['price'] + " * " + product['count'] + '<spring:message code="package.productpackage.months" />';
